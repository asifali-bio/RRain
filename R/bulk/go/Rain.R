...
ggplot(coords, aes(x = UMAP1, y = UMAP2, label = species)) +
  ...
  geom_text(vjust = -0.5) +
  theme_classic() +
  labs(title = "UMAP of species by GO composition")
...
ggplot(coords, aes(x = PC1, y = PC2, label = species)) +
  ...
  geom_text(vjust = -0.5) +
  theme_classic() +
  labs(title = "PCA of species by GO composition")
...
long$z = as.numeric(factor(long$go_id, levels = rownames(new4)))
...
p = p %>% layout(
  title = "3D Species Towers (GO composition)",
  scene = list(
    xaxis = list(title = "PC1"),
    yaxis = list(title = "PC2"),
    zaxis = list(title = "GO index")
  )
)
